...
function _colordiff(a_lab::Lab{T}, b_lab::Lab{T}, m::DE_2000) where T
    ...
    a_lab_r = Lab{F}(a_lab.l, muladd(a_lab.a, g, a_lab.a), a_lab.b)
    b_lab_r = Lab{F}(b_lab.l, muladd(b_lab.a, g, b_lab.a), b_lab.b)

    # Convert to L*C*h, where the remainder of the calculations are performed
    a = convert(LCHab{F}, a_lab_r)
    b = convert(LCHab{F}, b_lab_r)
    ...
    # lightness weight
    mls = (ml - 50)^2
    sl = 1 + F(0.015) * mls / _sqrt(20 + mls)

    # chroma weight
    sc = 1 + F(0.045) * mc

    # hue weight
    sh = 1 + F(0.015) * mc * _de2000_t(mh)
...
@inline function _de2000_t(mh::Float32)
    if mh < 64.0f0
